...
async def echo(bot, update):
    if update.from_user.id in Config.BANNED_USERS:
        await update.reply_text("𝕃𝕠𝕝, 𝕪𝕠𝕦 𝕒𝕣𝕖 𝕓𝕒𝕟𝕟𝕖𝕕.")
        return
    ...
    if update_channel:
        try:
            ...
            await update.reply_text("Oops! Something Wrong. Contact my Support Group")
            return
    ...
    if url.startswith("https://mega.nz"):
        ...
        LOCATION = "./"
        msg = await update.reply_text("⏳ 𝗗𝗼𝘄𝗻𝗹𝗼𝗮𝗱𝗶𝗻𝗴...")
        try:
            file = m.download_url(url, LOCATION)
        except Exception as e:
            print(str(e))
            return await msg.edit("❌ Invalid Link.")
        await msg.edit("📤 𝗨𝗽𝗹𝗼𝗮𝗱𝗶𝗻𝗴...")
        cap = f" 𝗨𝗽𝗹𝗼𝗮𝗱𝗲𝗱 𝗯𝘆 @AIOProBot"
        await bot.send_document(update.chat.id, file, caption=cap)
        await msg.delete()
        os.remove(file)
    else:
